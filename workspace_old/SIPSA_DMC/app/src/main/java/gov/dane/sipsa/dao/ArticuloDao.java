...
public class ArticuloDao extends AbstractDao<Articulo, Long> {
    ...
    */
    ...
    private Query<Articulo> tipoRecoleccion_ArtiTipoRecoleccionQuery;
    private Query<Articulo> casaComercial_ArtiCasaComercialQuery;
    ...
    public static void createTable(SQLiteDatabase db, boolean ifNotExists) {
        ...
        if(ifNotExists){
            Cursor cursor = db.rawQuery("pragma table_info(ARTICULO)", null);
            if (cursor != null) {
                if (cursor.getCount() > 0) {
                    ...
                    if(!campo1)db.execSQL("ALTER TABLE ARTICULO ADD COLUMN TIPO TEXT");
                    if(!campo2)db.execSQL("ALTER TABLE ARTICULO ADD COLUMN FRECUENCIA TEXT");
                    if(!campo3)db.execSQL("ALTER TABLE ARTICULO ADD COLUMN UNIDAD TEXT");
                    if(!campo4)db.execSQL("ALTER TABLE ARTICULO ADD COLUMN OBSERVACION TEXT");
                }
                cursor.close();
            }
        }
    ...
    @Override
    protected void attachEntity(Articulo entity) {
    ...
    public List<Articulo> _queryTipoRecoleccion_ArtiTipoRecoleccion(Long tireId) {
        synchronized (this) {
            if (tipoRecoleccion_ArtiTipoRecoleccionQuery == null) {
                QueryBuilder<Articulo> queryBuilder = queryBuilder();
                queryBuilder.where(Properties.TireId.eq(null));
                tipoRecoleccion_ArtiTipoRecoleccionQuery = queryBuilder.build();
            }
        }
        Query<Articulo> query = tipoRecoleccion_ArtiTipoRecoleccionQuery.forCurrentThread();
        query.setParameter(0, tireId);
        return query.list();
    }

    /** Internal query to resolve the "artiCasaComercial" to-many relationship of CasaComercial. */
    public List<Articulo> _queryCasaComercial_ArtiCasaComercial(Long cacoId) {
        synchronized (this) {
        ...
        Query<Articulo> query = casaComercial_ArtiCasaComercialQuery.forCurrentThread();
