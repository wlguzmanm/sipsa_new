 ...
 */
public class ResumenFuentesActivity extends App {
    private IDatabaseManager databaseManager;
    private List<Fuente> fuentes = new ArrayList<Fuente>();
    FuenteResumenRecyclerAdapter fuenteAdapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        fuentes  = databaseManager.listaFuentesByTransmitidoEstado(false);


        transmisionLocal = databaseManager.getConfiguracion().getTransmisionLocal();

        fuenteAdapter = new FuenteResumenRecyclerAdapter(fuentes);
                        ...
                        if (Util.isNetworkAvailable(getApplicationContext())) {
                            task_transmitirHttp = new AsyncTask<Void, Void, Status>() {
                                ...
                                protected void onPreExecute() {
                                    pdialog = ProgressDialog.show(ResumenFuentesActivity.this, "",
                                ...
                                protected gov.dane.sipsa.model.Status doInBackground(Void... params) {
                                    ...
                                    if (usuario != null) {
                                        EnvioRecoleccion envioRecoleccion = new EnvioRecoleccion();
                                        envioRecoleccion.setIdPersona(usuario.getUspeID().toString());
                                        envioRecoleccion.setFuente(databaseManager.listaFuenteArticuloByTransmitidoEstado());
                                        envioRecoleccion.setRecoleccion(databaseManager.listaRecoleccionTransmitir());

                                        /*String stringFile1 = toJson(envioRecoleccion);
                                        guardarBackup(stringFile1);*/

                                        gov.dane.sipsa.model.Status _status = RestServices.getInstance().enviarRecoleccion(envioRecoleccion, Integer.parseInt(usuario.getUspeID().toString()));
                                        ...
                                        if (_status.getType().equals(gov.dane.sipsa.model.Status.StatusType.OK)) {
                                            List<FuenteArticulo> fuentes = envioRecoleccion.getFuente();

                                            for (Recoleccion recoleccion : envioRecoleccion.getRecoleccion()) {
                                                recoleccion.setTransmitido(true);
                                                databaseManager.save(recoleccion);
                                            }

                                            for (FuenteArticulo fuente : fuentes) {

                                                //  fuente.setTransmitido(true);
                        ...
                        task_transmitirHttp = new AsyncTask<Void, Void, Status>() {
                            ...
                            protected gov.dane.sipsa.model.Status doInBackground(Void... params) {
                                ...
                                if (usuario != null) {
                                    ...
                                    String FILE_NAME = "T_IN_"+usuario.getUsuario()+"_"+year+"_"+month+"_"+day;
    ...
    private void guardarBackup(String stringFile) {
        ...
        Log.e("tag", getLogPath() + "/" + FOLDER_NAME+"/"+ FILE_NAME.toString());
        OutputStream out = null;
        try {
